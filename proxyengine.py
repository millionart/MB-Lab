...
class ProxyEngine:
    ...
    def transfer_weights(body, proxy):
        ...
        if fit_shapekey:
            ...
            algorithms.print_log_report("WARNING", "在没有拟合的情况下执行重量转移（找不到拟合的形状键）")
    ...
    def fit_near_vertices(self, basis_proxy, basis_body, proxy_shapekey, current_body, proxy_threshold=0.025):
        ...
        valid_polygons_indxs = algorithms.load_json_data(polygons_path, "用于代理拟合的多边形子集")
    ...
    def fit_proxy_object(self, proxy_offset=0.0, proxy_threshold=0.5, create_proxy_mask=False, transfer_w=True):
        ...
        if status == "OK":
            ...
            algorithms.print_log_report("INFO", "正在过滤替代物 {0}".format(proxy.name))
            ...
            for polyg in obj.data.polygons:
                ...
                if max_deform > threshold:
                    for idx in polyg.vertices:
                        ...
                        # + fitted_forma.vertices[idx].normal*difference.length
                        shape_to_finish.data[idx].co = corrected_position
